...
public class TabelaHash {
    ...
    public Object findElement(int k) {
        ...
        do {
            ...
            } else if (vetor[(h + d) % tamanho].getChave() == k) {
                return vetor[(h + d) % tamanho].getElemento();
            } else {
                d = d + d;
            }
            cont++;
    ...
    public void insertItem(int k, Object o) {
        ...
        if (size() == tamanho) {
            tamanho = tamanho * 2;
            while (isPrime(tamanho) == false) {
                tamanho++;
            }
            System.out.println(
            "Tabela cheia. Aumentando... Novo tamanho = " + tamanho);
TabelaHash aux = new TabelaHash(tamanho);
        ...
        if (vetor[h] == null || vetor[h].getElemento() == "AVAILABLE") {
            ...
            do {
                if (vetor[(h + d) % tamanho] == null
                        || vetor[(h + d) % tamanho].getElemento() == "AVAILABLE") {
                    Item i = new Item(k, o);
                    vetor[(h + d) % tamanho] = i;
                    break;
                }
    ...
    public Object removeElement(int k) {
        ...
        do {
            if (vetor[h] == null) {
                ...
                Object aux = vetor[(h + d) % tamanho].getElemento();
                vetor[(h + d) % tamanho].setChave(0);
                vetor[(h + d) % tamanho].setElemento("AVAILABLE");
                tamanho_atual--;
                return aux;
            } else {
                d = d + d;
